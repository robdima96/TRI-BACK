...
TRADITIONAL_RAG_FRAMEWORK_PATH = (
    _PROMPTS_DIR / "traditional_rag_reasoning_framework.md"
)
...
@lru_cache(maxsize=2)
def _load_framework_text(path: Path) -> str:
...
def render_system_prompt(
    *,
    ...
    touched_conditions: list[dict[str, object]],
    ...
    framework_path = FRAMEWORK_PATH if graph_enabled else TRADITIONAL_RAG_FRAMEWORK_PATH
    framework = _load_framework_text(framework_path)
    touched = (
        "\n".join(
            "- "
            + str(row["condition"])
            + ": "
            + str(row["factor_hit_count"])
            + " factor hit(s) ["
            + ", ".join(str(v) for v in row.get("matched_factors", []))
            + "]"
            for row in touched_conditions
        )
