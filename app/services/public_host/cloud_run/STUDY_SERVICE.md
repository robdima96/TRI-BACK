...
The **bot** service packaging lives in this git repo (`DigiMSKbot` on GitHub = the `bot/` folder).  
The **study UI** currently lives **outside** that repo:

```text
DigiMSKbot/                         workspace (not necessarily one git root)
  bot/                              <- git remote: github.com/robdima96/DigiMSKbot.git
  prototypes/digimsk_study_app/     <- Reflex study app (finish this for Cloud Run)
  Graphs/backups/red flags/v2/      <- uploaded to GCS by upload_gcs_assets.ps1
```
...
## 1. Make the study app buildable from a known context

Pick **one**:

**Option A (recommended for simplicity):** Copy or git-subtree the study app into the bot repo, e.g. `bot/study_app/`, and change `Dockerfile.study` `COPY` paths from `prototypes/digimsk_study_app/` to `study_app/`. Then Cloud Build from this repo can build both images.

**Option B:** Keep study outside; always build from the **workspace root** (`DigiMSKbot/`):
...
Option B does **not** work with “Connect repo” on `robdima96/DigiMSKbot` alone until the study tree is in that repo.
