...
def tool_list_candidate_conditions(ctx: ToolContext, **_: Any) -> ToolResult:
    """Deterministic ranked conditions from the ground-truth traversal."""
    from app.services.graphrag import match_checklist_to_factors, traverse_from_turn
    from app.services.rag.fusion import build_traversal_seeds

    seeds = build_traversal_seeds(
        checklist=ctx.checklist,
        chunk_matches=ctx.chunk_matches,
    )
    trace = traverse_from_turn(
        checklist=ctx.checklist,
        chunk_matches=ctx.chunk_matches,
        factor_matches=seeds.factor_matches,
        title="agentic:list_candidate_conditions",
    )
    ranked = [
        {"condition": r.condition, "risk_score": r.risk_score, "path_count": r.path_count}
        for r in trace.condition_risks
    ]
    obs = {
    ...
    return ToolResult(
        name="list_candidate_conditions",
        observation=_clip(json.dumps(obs, ensure_ascii=False)),
        data=obs,
        factors=list(trace.matched_factors),
...
_TOOL_SPECS: tuple[ToolSpec, ...] = (
    ...
    ToolSpec(
        name="list_candidate_conditions",
        description="Deterministic ground-truth ranking of candidate serious conditions for this turn.",
        arguments={},
        func=tool_list_candidate_conditions,
    ),
    ToolSpec(
        name="get_factor_paths",
...
def tool_names() -> list[str]:
    return [t.name for t in _TOOL_SPECS]
...
def invoke_tool(name: str, arguments: dict[str, Any], ctx: ToolContext) -> ToolResult:
    """Dispatch a validated tool call; unknown tools return an error result."""
    ...
    if spec is None:
        return ToolResult(
            ...
            error="unknown tool",
            observation=(
                f"Unknown tool {name!r}. Available: " + ", ".join(_TOOLS_BY_NAME)
            ),
        )
