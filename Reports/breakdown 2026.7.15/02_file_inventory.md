# DigiMSK — File Inventory

Short map of **where the main pieces live**. You do not need to open every folder.

## Two main pieces

- **Study website** (`prototypes/digimsk_study_app/`) — Login, chat, three display arms, feedback, research logging. Talks only to the bot; does **not** call the AI model itself.
...
- **Wording backend** — `DIGIMSK_GENERATOR_BACKEND` = `vertex` or `local`
- **Vertex project** — `DIGIMSK_VERTEX_PROJECT_ID` (and location); local model folder only if `local`
