...
from digimsk_study_app.config import INTRO_DELAY_SEC, INTRO_MESSAGE, INTRO_MESSAGE_ID
from digimsk_study_app.state.chat_state import build_intro_message, should_play_intro


def test_intro_constants():
    assert INTRO_MESSAGE_ID == "msg_intro"
    assert INTRO_DELAY_SEC == 10
    assert "DigiMSK" in INTRO_MESSAGE
